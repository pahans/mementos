 ...
 */
function PictureGrid({ selectedPics = [], onChange, columnCount = 3 }) {
  const pictureChunks = chunk(selectedPics, columnCount);
  const move = (from, to) => {
    const indexTo = selectedPics.findIndex((item) => item.id === to);
    const indexFrom = selectedPics.findIndex((item) => item.id === from);
